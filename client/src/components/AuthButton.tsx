...
function loginWithProvider(provider: 'google' | 'github') {
  ...
  window.location.href = `${API_URL}/api/auth/sign-in/${provider}?callbackURL=${callbackURL}`;
}
...
export const AuthButton: React.FC = () => {
  ...
  return (
    <div className="auth-user" onClick={() => setMenuOpen(v => !v)}>
      {session.user.image
        ? <img src={session.user.image} alt="" className="auth-avatar" />
        : <div className="auth-avatar-placeholder">{session.user.name?.[0] ?? '?'}</div>
      }
      <span className="auth-name">{session.user.name?.split(' ')[0]}</span>
      <span className="auth-chevron">{menuOpen ? '▴' : '▾'}</span>
      {menuOpen && (
