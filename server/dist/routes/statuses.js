...
router.get('/', async (req, res) => {
    ...
    const { rows } = await pool.query(`SELECT game_id, status FROM game_statuses
     WHERE user_id = $1 AND status != 'none'`, [user.id]);
    // Return as { gameId: status } map — same shape as frontend useState
    const statusMap = {};
    for (const row of rows)
        statusMap[row.game_id] = row.status;
...
router.post('/', async (req, res) => {
    ...
    if (status === 'none') {
        await pool.query(`DELETE FROM game_statuses WHERE user_id = $1 AND game_id = $2`, [user.id, gameId]);
    }
    else {
        await pool.query(`INSERT INTO game_statuses (user_id, game_id, status, updated_at)
       VALUES ($1, $2, $3, NOW())
       ON CONFLICT (user_id, game_id)
       DO UPDATE SET status = $3, updated_at = NOW()`, [user.id, gameId, status]);
    }
